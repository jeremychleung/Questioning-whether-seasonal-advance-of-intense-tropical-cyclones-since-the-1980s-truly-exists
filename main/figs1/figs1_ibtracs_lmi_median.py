...
ax1.text(-0.19,1.02,chr(101),fontdict={'weight':'bold','size':16}, transform=ax1.transAxes)
ax2.text(-0.19,1.02,chr(102),fontdict={'weight':'bold','size':16}, transform=ax2.transAxes)
ax3.text(-0.19,1.02,chr(103),fontdict={'weight':'bold','size':16}, transform=ax3.transAxes)
ax4.text(-0.19,1.02,chr(104),fontdict={'weight':'bold','size':16}, transform=ax4.transAxes)
...
for iw,w in enumerate([110,115]):
    ...
    for iyr,yr in enumerate(years):
        ...
        yr50th[iyr] = np.percentile(df50th['occurtime'].values,50)
    ...
    sns.regplot(x=x, y=y,ci=95,color=lc[iw],scatter=False,line_kws={'linewidth':2},seed=10,ax=ax1)
    ...
    ax1.set_ylim([224,297])
    ax1.invert_yaxis()
    ax1.set_title('IBTrACS, NH', loc='left',fontsize=9,pad=3)
    ...
    print('dof:',dof)
    print(f'IBTrACS-NH slope:{slope*10:+.2f}±{ppf*std_err*10:.2f}, mk-test:{pvalue:.3f}, t-test:{pvalue_ls:.3f}')
...
for iw,w in enumerate([110,115]):
    ...
    sns.regplot(x=x, y=y,ci=95,color=lc[iw],scatter=False,line_kws={'linewidth':2},seed=20,ax=ax3)
    ...
    ax3.set_ylim([178,289])
    ax3.invert_yaxis()
    ax3.set_title('IBTrACS, SH', loc='left',fontsize=10,pad=3)
    ...
    ppf = stats.t.ppf(1 - 0.05/2, dof)
    print('dof:',dof)
    print(f'IBTrACS-SH slope:{slope*10:+.2f}±{ppf*std_err*10:.2f}, mk-test:{pvalue:.3f}, t-test:{pvalue_ls:.3f}')
...
ax2.set_xlim([30,140])
ax2.set_ylim([-15,7])
ax2.set_title('IBTrACS, NH', loc='left',fontsize=10,pad=3)
...
ax4.set_xlim([30,140])
ax4.set_ylim([-20,15])
ax4.set_title('IBTrACS, SH', loc='left',fontsize=10,pad=3)

#%%
fig.savefig('figs1_ibtracs_lmi_median.png', dpi=500)

#p50ns = pd.DataFrame({'year':years,'NH':p50n,'SH':p50s})
#p50ns.to_csv('sphere_median.csv', index=False)