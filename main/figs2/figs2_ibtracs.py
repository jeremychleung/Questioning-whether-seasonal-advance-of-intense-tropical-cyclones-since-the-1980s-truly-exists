...
for isph,sph in enumerate(['NH','SH']):
    ...
    if sph == 'NH':
        cond1 = ibt['basin'].isin(['NA','EP','WP'])
    elif sph == 'SH':
        cond1 = ibt['basin'].isin(['SI','SP'])
    ...
    for imo,mo in enumerate(months):
        ...
        for iyr,yr in enumerate(years):
            ...
            df = ibt[cond].reset_index(drop=True)
    ...
    if sph == 'NH':
        ...
        ax.set_yticks(np.arange(-0.5,0.9,0.2))
    ...
    ax.text(-0.15,1.01,chr(99+isph),transform=ax.transAxes,fontdict={'weight':'bold','size':25})

fig.savefig('figs2_ibtracs.png', dpi=500)