...
for iob,ob in enumerate(oceanbasin):
    df = pd.read_csv('../fig1/itc_in_datasets.'+ob.lower()+'.csv')
    ...
    ax.set_title(ob,fontsize=13,pad=4.2)
    ...
    ax.text(-0.10,1.01,chr(97+iob),transform=ax.transAxes,fontdict={'weight':'bold','size':21})

    fig.savefig('figs5_'+ob.lower()+'.png', dpi=600)
    plt.show()
    fig.clf()