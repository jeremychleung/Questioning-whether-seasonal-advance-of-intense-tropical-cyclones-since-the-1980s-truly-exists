...
for i,isid in enumerate(tc_sids):
    ...
    for k in range(2):
        ax = axs[k]
        ax.text(-0.10,1.06,chr(101+k), transform=ax.transAxes, fontdict={'weight':'bold','size':17})

        m = Basemap(llcrnrlon=110.,llcrnrlat=-28.01,urcrnrlon=140.857,urcrnrlat=-10,projection='mill',\
        ...
        m.drawmeridians(np.arange(-180,180,10),labels=[0,0,0,1], fontsize=8, linewidth=0.5)
        ...
        for ii in range(len(colorlist)):
            ...
            ax.text(0.89,0.95-ii*0.061-0.018,labellist[ii],fontsize=6.0,transform=ax.transAxes,zorder=8)
