# -*- coding: utf-8 -*-
"""
Created on Mon Jan  6 10:51:55 2025
...
for iob,ob in enumerate(oceanbasin):
    ...
    for iyr,yr in enumerate(years):
        ...
        adt_yr = adt[cond].reset_index(drop=True)
        if adt_yr.shape[0]:
    ...
    for iyr,yr in enumerate(years):
        ...
        ibt_yr = ibt[cond].reset_index(drop=True)
        if ibt_yr.shape[0]:
...
df.to_csv('oceanbasin_median.csv', index=True)